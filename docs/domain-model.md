...
| 用語            | 英語名                 | 意味                                                                          |
| --------------- | ---------------------- | ----------------------------------------------------------------------------- |
| 機器            | Equipment              | 点検・校正の対象となる測定器・設備                                            |
...
| 実施記録        | InspectionRecord       | 項目の実施履歴。実施のたびに次回期限が更新される                              |
| 外部校正案件    | CalibrationOrder       | 外部校正の発注1件。発注〜返却までの進捗と納期を追跡                           |
...
erDiagram
    ...
    InspectionItem ||--o{ InspectionRecord : "実施履歴"
    InspectionItem ||--o{ CalibrationOrder : "外部校正案件(外部のみ)"
...
4. **外部校正案件一覧** — 案件の進捗管理(かんばん or リスト)
5. **実施記録登録** — 項目からの記録入力
