...
Vitest + Testing Library を基本とし、`domain/` 配下の純粋関数群に対しては fast-check による property test を組み合わせる方針とする。特に `addCycle`（暦月ベースの次回期限計算。domain-model.md §4.1）の月末補正や、`deriveItemStatus`（優先度付きステータス判定。domain-model.md §4.3）のような「境界値・優先順位が絡む純粋ロジック」は例示ベースのテストだけでは見落としが生じやすいため、property testとの相性が良いと考える。詳細な対象ファイルは [store.md](./store.md) のテスト節を参照。
