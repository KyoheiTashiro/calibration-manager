...
関連: [domain-model.md](../../spec/domain-model.md) ／ [tech-stack.md](./tech-stack.md)
...
- 各エンティティの属性一覧は本書では再掲しない。[domain-model.md](../../spec/domain-model.md) §3を参照。
...
3段構え（ハッピーパス → 部分破損サルベージ → 最終手段）で復元する。
...
- `deriveServiceItemStatus(serviceItem, serviceOrders, today)`（`domain/serviceItemStatus.ts`）— domain-model.md §4.3の優先度付き5ステータス
