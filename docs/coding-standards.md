...
calibration-managerのコード慣習。**実装・レビュー時はこれに従う**。原則として姉妹プロジェクト pinpon-match-manage の規約を踏襲し「実際に守られている慣習」として明文化する。各ルールに根拠を添える（このコードベースは設定ファイルにまで「なぜ」を日本語コメントで残す文化のため）。
