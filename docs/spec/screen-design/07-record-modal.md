...
**起動元**: [機器詳細](./04-equipment-detail.md)、[点検校正項目一覧](./05-service-item-list.md)、[点検校正外部案件](./08-service-orders.md)の `returned` カード
...
| 案件参照   | serviceOrderId  |      | 案件経由起動時に自動セット                                |

**doneBy プリフィルの解決順(D-017)**: ①案件経由起動(serviceOrderId あり)→ 当該案件の vendorId から Vendor.name、②それ以外で項目が external → item.vendorId から Vendor.name、③internal または Vendor が解決不能 → 空欄。
...
5. **点検校正外部案件(status=`returned`)から起動した場合**: `serviceOrderId` を記録に紐付け、当該 ServiceOrder を `completed` へ遷移(§3.6)。
...
- `serviceOrderId` 指定時に、案件が存在しない、または現状態から `completed` へ遷移不可(`returned` 以外からの遷移)。
