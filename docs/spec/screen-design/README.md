...
- **共通レイアウト**:
  ...
  - 各画面は画面タイトル(h1)を表示する(D-095。D-088 は撤回)。利用マニュアルは「ページ上部へ戻る」(D-067)のスクロール先が h1。機器詳細・機器フォームは内容を伴う見出し(機器名など)を h1 とする。一覧画面の追加ボタンは検索・フィルタ行の右端に置き、通知一覧の「全て既読」はタブバー右端(Tabs の actions スロット)に置く。
...
- 確定ボタンの色: 破壊的操作は赤(`danger`)、非破壊の確認(通知の「全て既読」等)は青(`primary`)。ConfirmModal の `variant` で切り替える(既定は `danger`)(D-096)。
