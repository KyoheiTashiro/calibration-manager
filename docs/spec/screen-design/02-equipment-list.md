...
- 状態フィルタ(4択、D-082): `全て`(既定)/ `稼働` / `休止` / `廃棄`。フィルタ値は EquipmentStatus とは別軸の画面ローカル定数。旧「稼働+休止」複合値(D-010)は D-082 で廃止。
- 行クリック → `/equipment/:id`。
...
- ページネーション: [§0.8](./README.md#08-ページネーション共通方針d-076) に従う(検索・状態フィルタ適用後の結果に適用)。
