...
一覧画面の表示項目・フィルタ項目・行アクションは各画面設計([02](./screen-design/02-equipment-list.md) [05](./screen-design/05-service-item-list.md) [08](./screen-design/08-service-orders.md) 等)に従う。本書は共通の見た目とインタラクションパターンを定める。
...
- フォーム系モーダル(点検校正項目・実施記録登録・案件作成・メーカー/取引先・担当者の編集など): `max-w-lg`〜`max-w-2xl`。フッターは右寄せで「保存」のみ(キャンセルボタンは置かず、閉じる操作はヘッダー右上の×・Esc・オーバーレイクリック。§0.5)。
