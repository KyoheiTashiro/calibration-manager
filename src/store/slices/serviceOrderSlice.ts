...
import { type ServiceOrder, ORDER_STATUS, type OrderStatus } from "@/store/types";
import { createId } from "@/utils/id";
import { recordValue } from "@/utils/record";

export type AddOrderInput = Omit<ServiceOrder, "id" | "status">;
...
export type ServiceOrderSlice = {
  ...
  updateOrder: (id: string, patch: Partial<Omit<ServiceOrder, "id" | "status">>) => void;
  /**
...
export const createServiceOrderSlice: AppSliceCreator<ServiceOrderSlice> = (set, get) => ({
  ...
  addOrder: (input): string | null => {
    ...
    const hasActiveOrder = Object.values(orders).some(
      (order) =>
        order.serviceItemId === input.serviceItemId && isActiveOrderStatus(order.status),
    );
