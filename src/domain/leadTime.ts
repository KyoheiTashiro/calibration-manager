...
/**
 * 納期（日数）を解決する。item 側の個別納期を優先し、未設定なら依頼先の標準納期へ
...
export const resolveLeadTime = (
  item: Pick<InspectionItem, "leadTimeDays">,
  vendor: Pick<Vendor, "standardLeadTimeDays"> | null,
): number | null => item.leadTimeDays ?? vendor?.standardLeadTimeDays ?? null;

/**
 * 発注推奨日を返す。計算できない場合は null（例外を投げない。coding-standards.md §8）:
 * - 内部実施の項目（発注の概念がない。domain-model.md §4.2「外部のみ」）
 * - 納期が item・vendor のどちらからも解決できない
 * - nextDueDate が日付として不正
 */
export const recommendedOrderDate = (
  item: Pick<InspectionItem, "execution" | "leadTimeDays" | "bufferDays" | "nextDueDate">,
  vendor: Pick<Vendor, "standardLeadTimeDays"> | null,
): IsoDateString | null => {
  if (item.execution !== EXECUTION.EXTERNAL) return null;
  const leadTimeDays = resolveLeadTime(item, vendor);
  if (leadTimeDays === null) return null;
  return addDays(item.nextDueDate, -(leadTimeDays + item.bufferDays));
};
