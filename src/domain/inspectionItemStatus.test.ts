...
describe("deriveInspectionItemStatus（優先度: overdue > orderNow > inProgress > dueSoon > ok）", () => {
  it("今日 > nextDueDate なら overdue", () => {
    expect(deriveInspectionItemStatus(buildInspectionItem(), [], null, "2026-08-01")).toBe(
      INSPECTION_ITEM_STATUS.OVERDUE,
    );
  });

  it("今日 = nextDueDate は overdue ではない（厳密な超過のみ）", () => {
    expect(deriveInspectionItemStatus(buildInspectionItem(), [], null, "2026-07-31")).not.toBe(
      INSPECTION_ITEM_STATUS.OVERDUE,
    );
  });
  ...
  it("優先度の逆転がない: overdue かつ orderNow の条件を両方満たす場合は overdue が勝つ", () => {
    ...
    expect(deriveInspectionItemStatus(buildInspectionItem(), [], null, "2026-09-01")).toBe(
      INSPECTION_ITEM_STATUS.OVERDUE,
    );
  });

  it("外部・今日 ≥ 発注推奨日・有効案件なしなら orderNow（境界: 推奨日当日）", () => {
    expect(deriveInspectionItemStatus(buildInspectionItem(), [], null, "2026-07-07")).toBe(
  ...
  it("発注推奨日の前日は orderNow にならない（この例では dueSoon 窓にも入っていない）", () => {
    ...
    expect(deriveInspectionItemStatus(buildInspectionItem(), [], null, "2026-07-06")).toBe(
  ...
  it("orderNow は有効な案件（planned 含む）があると成立しない", () => {
    ...
    expect(deriveInspectionItemStatus(buildInspectionItem(), planned, null, "2026-07-07")).toBe(
      INSPECTION_ITEM_STATUS.DUE_SOON,
    );
  });
  ...
  it("completed / cancelled の案件は「有効な案件」ではないので orderNow が成立する", () => {
    ...
    expect(deriveInspectionItemStatus(buildInspectionItem(), finished, null, "2026-07-07")).toBe(
      INSPECTION_ITEM_STATUS.ORDER_NOW,
    );
  });
  ...
  it("外部・ordered の案件があれば inProgress", () => {
    ...
    expect(deriveInspectionItemStatus(buildInspectionItem(), orders, null, "2026-07-07")).toBe(
  ...
  it("他項目の案件は判定に影響しない（inspectionItemId で絞り込む）", () => {
    ...
    expect(
      deriveInspectionItemStatus(
        buildInspectionItem(),
        otherInspectionItemOrders,
        null,
        "2026-07-07",
      ),
    ).toBe(INSPECTION_ITEM_STATUS.ORDER_NOW);
  });
  ...
  it("今日 ≥ nextDueDate − noticeDaysBefore なら dueSoon（境界: 開始日当日）", () => {
    ...
    expect(deriveInspectionItemStatus(internalInspectionItem, [], null, "2026-07-01")).toBe(
      INSPECTION_ITEM_STATUS.DUE_SOON,
    );
  });
  ...
  it("期限当日は dueSoon（overdue ではない）", () => {
    ...
    expect(deriveInspectionItemStatus(internalInspectionItem, [], null, "2026-07-31")).toBe(
      INSPECTION_ITEM_STATUS.DUE_SOON,
    );
  });
  ...
  it("dueSoon 開始日の前日は ok", () => {
    ...
    expect(deriveInspectionItemStatus(internalInspectionItem, [], null, "2026-06-30")).toBe(
  ...
  it("内部実施は orderNow / inProgress にならない（発注の概念がない）", () => {
    ...
    const orders = [buildOrder(ORDER_STATUS.ORDERED)];
    expect(deriveInspectionItemStatus(internalInspectionItem, orders, null, "2026-07-07")).toBe(
      INSPECTION_ITEM_STATUS.DUE_SOON,
    );
  });
  ...
  it("inspectionItem に納期が無くても vendor の標準納期で orderNow を判定できる（フォールバック）", () => {
    ...
    expect(
      deriveInspectionItemStatus(inspectionItem, [], { standardLeadTimeDays: 10 }, "2026-07-07"),
    ).toBe(INSPECTION_ITEM_STATUS.ORDER_NOW);
  });

  it("納期がどこからも解決できない外部項目は orderNow にならない（推奨日を計算できない）", () => {
    const inspectionItem = buildInspectionItem({ leadTimeDays: undefined });
    expect(deriveInspectionItemStatus(inspectionItem, [], null, "2026-07-07")).toBe(
      INSPECTION_ITEM_STATUS.DUE_SOON,
    );
  });
});
