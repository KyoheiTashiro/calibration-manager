...
describe("RecordModal", () => {
  it("対象が「対象:管理番号 機器名 / 項目名」で固定表示される", () => {
    renderWithStore(<RecordModal open itemId={itemExternal.id} onClose={vi.fn()} />);
    expect(screen.getByText("対象:EQ-001 ノギス / 年次校正")).toBeInTheDocument();
  });

  it("項目が解決できない場合でも例外を投げず defensive 表示になる", () => {
    renderWithStore(<RecordModal open itemId="missing" onClose={vi.fn()} />);
  ...
  it("doneBy プリフィル: order 経由起動は案件の業者名", () => {
    renderWithStore(
      <RecordModal open itemId={itemExternal.id} orderId={orderReturned.id} onClose={vi.fn()} />,
    );
  ...
  it("doneBy プリフィル: external 項目(order なし)は項目の業者名", () => {
    renderWithStore(<RecordModal open itemId={itemExternal.id} onClose={vi.fn()} />);
    expect(screen.getByLabelText("実施者", { exact: false })).toHaveValue(itemVendor.name);
  });

  it("doneBy プリフィル: internal 項目は空欄", () => {
    renderWithStore(<RecordModal open itemId={itemInternal.id} onClose={vi.fn()} />);
  ...
  it("未来日を入力すると警告を表示するが登録はブロックしない", async () => {
    const user = userEvent.setup();
    const onClose = vi.fn();
    renderWithStore(<RecordModal open itemId={itemExternal.id} onClose={onClose} />);
  ...
  it("addRecord が no-op(null)の場合はエラーを表示しモーダルを閉じない", async () => {
    ...
    renderWithStore(
      <RecordModal open itemId={itemExternal.id} orderId={orderPlanned.id} onClose={onClose} />,
    );
