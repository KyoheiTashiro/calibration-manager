/**
 * 項目一覧画面(screen-design/05-item-list.md)固有のフィルタ解釈・適用ロジック。
 * UI(index.tsx)を薄いビューに保つため純関数へ切り出す(coding-standards.md §2)。
 * 行導出(itemRowsOf)はダッシュボードと共有する横断 selector のため store/selectors.ts に置く(D-024)。
 *
 * - フィルタの真実源は URL クエリ(D-022)。不正・未知値は「全て」扱いでエラーにしない。
 * - 表示対象は稼働(active)機器の有効(isActive=true)項目のみ。無効・非稼働トグルは不実装(D-023)。
 * - ステータスは保存値ではなく導出値(itemRowsOf 内 deriveItemStatus)。フィルタ status も導出結果に適用(§5)。
 */

import { ITEM_STATUS, type ItemStatus } from "@/domain/itemStatus";
import type { ItemRow } from "@/store/selectors";
import { EXECUTION, ITEM_TYPE, type Execution, type ItemType, type Person } from "@/store/types";
 ...
 * 行導出(itemRowsOf)はダッシュボードと共有するため store/selectors.ts に置く(D-024)。
 */
