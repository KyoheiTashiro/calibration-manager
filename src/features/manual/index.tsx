...
/* エクスポートファイル名一覧表(D-065)。別コンポーネントに切り出すことで、検索対象を
   囲む div の追加によって深くなった Manual 側の JSX ネストを抑える(react/jsx-max-depth)。
   行順は CSV_ENTITY_KINDS(推奨インポート順)。列仕様タブ・インポート順の記載と共通(D-058 / D-060 / D-065) */
const CsvFileNameTable = (): ReactElement => (
  <Table>
    <TableHead>
      <tr>
        <Th>データの種類</Th>
    ...
    <div className='flex flex-col gap-4'>
      ...
      <div ref={searchContentRef} className='flex flex-col gap-4'>
        ...
        >
          ...
          <p>
            データはすべてお使いのブラウザー内に保存されます。外部のサーバーには
        ...
        >
          ...
          <p>
            データの種類ごとに、1つのCSVファイルをダウンロードできます。ファイルはExcelでもそのまま
            開けます。データが1件もない場合でも、1行目の英語の項目名だけのCSVを出力できます。
          </p>
          <p>
        ...
        >
          ...
          <p>
            ...
            お使いのブラウザー内にのみ保存され、外部のサーバーには送信されません。
