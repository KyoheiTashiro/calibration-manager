 ...
 *
 * - 集計対象は itemRowsOf(@/store/selectors)の結果。稼働機器×有効項目×導出ステータスを
 *   一元化済みのため、ここでは status を数える・絞る・並べるだけを担う(導出ロジックは再実装しない)。
 * - 要対応リストの並び: 優先度 overdue → orderNow → dueSoon。同グループ内は itemRowsOf が保証する
 ...
 * itemRowsOf が nextDueDate 昇順・id 昇順を保証済みのため、優先度の安定ソートのみ追加する。
 */
