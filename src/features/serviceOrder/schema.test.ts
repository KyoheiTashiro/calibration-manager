...
import { orderDialogSchema, returnDialogSchema } from "@/features/serviceOrder/schema";
...
describe("orderDialogSchema", () => {
  ...
  it("発注日必須・任意項目空欄で通る", () => {
    expect(orderDialogSchema.safeParse(base).success).toBe(true);
  });

  it.each(["", "12000", "0"])("費用 %s は通る", (cost) => {
    expect(orderDialogSchema.safeParse({ ...base, cost }).success).toBe(true);
  });

  it.each(["1.5", "-1", "abc"])("費用 %s は整数粒度違反で弾く(D-021)", (cost) => {
    expect(orderDialogSchema.safeParse({ ...base, cost }).success).toBe(false);
  });

  it("発注日が空・形式不正なら弾く", () => {
    expect(orderDialogSchema.safeParse({ ...base, orderedDate: "" }).success).toBe(false);
    expect(orderDialogSchema.safeParse({ ...base, orderedDate: "2026-02-30" }).success).toBe(false);
  });
});
